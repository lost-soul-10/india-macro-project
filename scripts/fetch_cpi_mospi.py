...
def pick_2024_headline_row(rows: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """
    Keep the All India + Combined top-line row.
    Based on your successful Postman request, 2024 still works when level=subgroup
    and division=0 are passed, but the returned headline row itself has null lower fields.
    """
    for row in rows:
        if (
            ...
            and row.get("inflation") is not None
        ):
            return row
...
def fetch_2024_month(token: str, year: int, month_code: int) -> Optional[Dict[str, Any]]:
    ...
    if headline_row is None:
        print(f"No suitable headline CPI row found for {year}-{month_code:02d}")
